...
class QuizEngine {
  constructor() {
    ...
    // qIndex -> 'answered' | 'not-answered' | 'marked-review' | 'answered-marked' | 'unvisited'
    this.questionStatuses = {};
    this.isAnswerChecked = false;
    this.timerInterval = null;
    this.timeRemainingSecs = 0;
    this.totalTimeSecs = 0;
    this.examEndAt = 0; // epoch ms deadline; survives sleep/wake drift
    this.examConfig = null;
    this._visibilityHandler = null;
  }

  // Stop any running exam timer. Called on every mode entry so an exam
  // countdown can never leak into practice (or vice versa).
  stopTimer() {
  ...
  speakCurrent() {
    ...
    if (!q) return;
    const opts = q.options.map((o, i) => `Option ${'ABCD'[i]}: ${o}`).join('. ');
    window.audioManager.speak(`${q.question}. ${opts}`);
  }
  ...
  renderPracticeQuestion() {
    ...
    // Empty hearts never block learning — the child takes a hug break and
    // taps once to refill. This keeps the HUD number honest.
    if (window.storageManager.data.profile.hearts <= 0) {
      card.innerHTML = `
        <div class="card-box" style="max-width: 560px; margin: 0 auto; text-align: center; padding: 48px 28px;">
          <div style="font-size: 64px; margin-bottom: 12px;">💖</div>
          <h2 style="font-size: 24px; font-weight: 900; color: #1e293b; margin-bottom: 8px;">Hug break!</h2>
          <p style="color: #64748b; font-size: 16px; margin-bottom: 24px;">
            Your hearts ran out. Take a deep breath, wiggle your toes — then tap below to fill them back up and keep going!
          </p>
          <button class="btn-tactile btn-green touch-44" style="font-size: 16px; padding: 14px 28px;" onclick="window.quizEngine.refillHearts()">
        ...
        <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 12px; margin-bottom: 18px;">
          <div>
            <span class="brand-badge" style="background: #e0f2fe; color: #0284c7; margin-bottom: 8px; display: inline-block;">
              ${window.SOFUtils.escapeHtml(q.topic)}
            </span>
            <h2 style="font-size: 20px; font-weight: 800; line-height: 1.4; color: #1e293b;">
              ${window.SOFUtils.escapeHtml(q.question)}
            </h2>
          </div>
          <button class="btn-tactile btn-blue touch-44" style="padding: 8px 14px; font-size: 14px; flex-shrink: 0;" onclick="window.quizEngine.speakCurrent()" aria-label="Read question aloud">
            🔊 Read
          </button>
        </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 12px; margin-top: 20px;" id="practice-options-grid">
          ${q.options.map((opt, idx) => `
            <button class="btn-tactile btn-secondary practice-opt-btn" id="p-opt-${idx}" style="text-align: left; justify-content: flex-start; padding: 14px 18px; font-size: 16px;" onclick="window.quizEngine.handlePracticeSelect(${idx})">
              ...
              <span>${window.SOFUtils.escapeHtml(opt)}</span>
            </button>
          `).join('')}
        </div>
    ...
    // Auto read-aloud only after the child has interacted once: mobile browsers
    // block speech synthesis without a user gesture, and unprompted audio
    // startles early readers.
    if (window.audioManager.hasInteracted) {
  ...
  refillHearts() {
    window.storageManager.setHearts(5);
    window.app.updateHUD();
  ...
  // ==========================================
  // Sample WITHOUT replacement: n distinct random questions from a pool.
  static samplePool(pool, n) {
  ...
  startOfficialExam(subject = 'IMO', isQuick = false) {
    ...
    const picked = QuizEngine.samplePool(corePool, coreCount)
      .concat(QuizEngine.samplePool(hotPool, hotCount));
    // Shuffle option order so static key positions are not guessable.
    this.questions = picked.map(q => window.SOFUtils.shuffleOptions(q));
    this.currentIndex = 0;
    this.selectedAnswers = {};
    this.questionStatuses = {};
    ...
    this.examConfig = { subject, count: this.questions.length, isQuick };
  ...
  startTimer() {
    this.stopTimer();
    // Epoch deadline: setInterval drifts under device sleep, so remaining
    // time is always recomputed from the wall clock.
    this.examEndAt = Date.now() + this.timeRemainingSecs * 1000;
    const tick = () => {
      this.timeRemainingSecs = Math.max(0, Math.round((this.examEndAt - Date.now()) / 1000));
    ...
    // Re-sync immediately when the tab wakes — no lost minutes to sleep.
    this._visibilityHandler = () => { if (!document.hidden) tick(); };
  ...
  // Live palette counts for the legend (official iON shows real numbers).
  countByStatus() {
    const counts = { answered: 0, 'not-answered': 0, 'marked-review': 0, 'answered-marked': 0, unvisited: 0 };
    for (let i = 0; i < this.questions.length; i++) {
      const st = this.questionStatuses[i] || 'unvisited';
  ...
  jumpToSection(toHot) {
    ...
    if (this.questionStatuses[target] === 'unvisited') {
      this.questionStatuses[target] = 'not-answered';
    }
    this.renderExamStage();
  }

  // Leaving a question without answering mirrors iON: visited, not answered.
  leaveCurrentExam() {
  ...
  renderExamStage() {
    ...
    if (!this.questions.length || !this.questions[this.currentIndex]) {
      container.innerHTML = `
        <div class="card-box" style="max-width: 560px; margin: 40px auto; text-align: center; padding: 40px 28px;">
          <div style="font-size: 56px; margin-bottom: 12px;">🧭</div>
          <h2 style="font-size: 22px; font-weight: 900; color: #1e293b;">No questions found</h2>
          <p style="color: #64748b; font-size: 15px;">Please pick a subject (IGKO, IMO, or NSO) and try again.</p>
        </div>`;
      return;
    }
          ...
          <div class="ion-question-col">
            <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #e2e8f0; padding-bottom: 10px; margin-bottom: 16px;">
              ...
              <div style="display: flex; gap: 8px;">
                <button class="btn-tactile btn-blue touch-44" style="padding: 4px 10px; font-size: 12px;" onclick="window.quizEngine.speakCurrent()" aria-label="Read question aloud">
                  🔊 Read Aloud
                </button>
                <span style="font-size: 12px; background: #e2e8f0; padding: 4px 8px; border-radius: 4px; font-weight: 700;">
                  Marks: +${this.getQuestionWeight(q)} | Neg: 0
                </span>
              </div>
            </div>

            <p class="ion-q-statement">${window.SOFUtils.escapeHtml(q.question)}</p>
            ...
            <div class="ion-options-group">
              ${q.options.map((opt, idx) => `
                <label class="ion-opt-row" id="ion-row-${idx}">
                  ...
                  <span class="ion-opt-text">(${optLetters[idx]}) ${window.SOFUtils.escapeHtml(opt)}</span>
                </label>
              `).join('')}
            </div>
          ...
          <div class="ion-palette-col">
            <div class="ion-candidate-bar">
              ...
              <div class="ion-candidate-details">
                <div class="ion-candidate-name">${window.SOFUtils.escapeHtml(window.storageManager.data.profile.name)}</div>
            ...
            <div class="ion-palette-legend">
              <div class="ion-legend-item">
                <div class="ion-legend-box" style="background: #2e7d32;">${counts.answered}</div>
                <span>Answered</span>
              </div>
              <div class="ion-legend-item">
                <div class="ion-legend-box" style="background: #c62828;">${counts['not-answered']}</div>
                <span>Not Answered</span>
              </div>
              <div class="ion-legend-item">
                <div class="ion-legend-box" style="background: #6a1b9a;">${counts['marked-review']}</div>
                <span>Marked Review</span>
              </div>
              <div class="ion-legend-item">
                <div class="ion-legend-box answered-marked-box">${counts['answered-marked']}</div>
                <span>Answ. & Marked</span>
              </div>
              <div class="ion-legend-item">
                <div class="ion-legend-box" style="background: #cbd5e1; color: #333;">${counts.unvisited}</div>
  ...
  onExamRadioChange(idx) {
    ...
    // Official iON keeps the review flag: answering a marked question yields
    // the fifth state "Answered & Marked for Review".
    this.questionStatuses[this.currentIndex] =
      this.questionStatuses[this.currentIndex] === 'marked-review' ? 'answered-marked' : 'answered';
    this.renderExamStage();
  }
  ...
  markExamReview() {
    // Answered + marked keeps both facts (iON purple-with-green-dot state).
    this.questionStatuses[this.currentIndex] =
      this.selectedAnswers[this.currentIndex] !== undefined ? 'answered-marked' : 'marked-review';
  ...
  confirmSubmitExam() {
    ...
    const answeredCount = (counts.answered || 0) + (counts['answered-marked'] || 0);
    const pending = totalCount - answeredCount;
    // In-UI confirmation (works offline, kid-readable, no native dialog).
    container.innerHTML = `
      <div class="card-box" style="max-width: 560px; margin: 40px auto; text-align: center; padding: 40px 28px;">
        <div style="font-size: 56px; margin-bottom: 12px;">📝</div>
        <h2 style="font-size: 24px; font-weight: 900; color: #1e293b; margin-bottom: 8px;">Submit your exam?</h2>
        <p style="color: #475569; font-size: 16px; margin-bottom: 6px;">
          You answered <strong>${answeredCount} of ${totalCount}</strong> questions.
        </p>
        <p style="color: ${pending > 0 ? '#b91c1c' : '#15803d'}; font-size: 15px; font-weight: 800; margin-bottom: 24px;">
          ${pending > 0 ? `${pending} question${pending === 1 ? '' : 's'} still unanswered.` : 'Every question is answered. Great job!'}
        </p>
        <div style="display: flex; justify-content: center; gap: 12px; flex-wrap: wrap;">
          <button class="btn-tactile btn-secondary touch-44" style="font-size: 15px; padding: 12px 22px;" onclick="window.quizEngine.renderExamStage()">
            ◀ Keep Checking
          </button>
          <button class="btn-tactile btn-green touch-44" style="font-size: 15px; padding: 12px 22px;" onclick="window.quizEngine.submitExam()">
            Submit Test ✅
          </button>
        </div>
      </div>
  ...
  submitExam() {
    ...
    this.questions.forEach((q, idx) => {
      const weight = this.getQuestionWeight(q);
      totalMarks += weight;
      const sel = this.selectedAnswers[idx];
      const isCorrect = sel === q.answer;
      if (isCorrect) {
        earnedMarks += weight;
      }
      // Record EVERY question — skipped items must count as misses so
      // weak-area stats never overestimate avoided topics.
      window.storageManager.recordAttempt(q, sel === undefined ? -1 : sel, isCorrect);
    });
            ...
            return `
              <div style="padding: 14px; border: 1px solid #e2e8f0; border-left: 5px solid ${isCorrect ? '#22c55e' : '#ef4444'}; border-radius: 8px; background: #fff;">
                <div style="display: flex; justify-content: space-between; font-size: 12px; font-weight: 800; color: #64748b; margin-bottom: 4px;">
                  <span>Q${idx + 1} • ${esc(q.topic)} • ${weight} mark${weight === 1 ? '' : 's'}</span>
                  <span style="color: ${isCorrect ? '#15803d' : '#b91c1c'}; font-size: 13px;">${isCorrect ? `✅ Correct (+${weight})` : '❌ Incorrect (0)'}</span>
                </div>
                <div style="font-weight: 700; font-size: 15px; margin-bottom: 6px;">${esc(q.question)}</div>
                <div style="font-size: 13px; color: #334155;">
                  <strong>Your answer:</strong> ${esc(chosenText)} | <strong>Correct answer:</strong> ${esc(correctText)}
                </div>
                <div style="font-size: 13px; color: #0369a1; margin-top: 6px;">💡 ${esc(q.explanation)}</div>
              </div>
            `;
          }).join('')}
        </div>
